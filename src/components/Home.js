...
function Home() {
  ...
  const handleAgreeAndContinue = () => {
    localStorage.setItem('hasAgreedToTerms', 'true'); // Mark that the user has agreed
    localStorage.removeItem('firstLogin'); // Remove first login flag
    setShowTermsModal(false); // Hide the terms modal
  };
  ...
  const generateInvoiceId = () => {
    ...
    return `QTN${month}${day}${hours}${minutes}${seconds}`;
  };
  ...
  return (
    <div className="App">
      ...
      <div style={{ textAlign: 'center', marginTop: '20px', marginBottom: '20px' }}>
        <button onClick={downloadInvoiceAsPDF}>Generate Quote</button>
      </div>
